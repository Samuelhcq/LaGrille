...
bool pionOrthogonalOrange (solution s, int x1, int y1, int x2, int y2)
{
    if (y1 == y2) return true;  //En dessous
    if (x1 == x2) return true; //A droite
    
    return false;
}

//Vérifie les pions en diagonale
bool pionDiagonalOrange (solution s, int x1, int y1, int x2, int y2)
{
    int difference = y2 - y1;
    
    if (x1 == x2-difference && y1 == y2-difference) return true; //En bas à droite
    if (x1 == x2+difference && y1 == y2-difference) return true; //En bas à gauche
    ...
    if (nbPionOrange >= 2)
    {
        ...
        for (int j = 0; j < nbPionOrange-1; ++j)
        {
            ...
            for (int h = j+1; h < nbPionOrange; ++h)
            {
                ...
                if (pionDiagonalOrange(s, position[j][0], position[j][1], position[h][0], position[h][1]) or pionOrthogonalOrange(s, position[j][0], position[j][1], position[h][0], position[h][1])) nbPaire++;
            }
        }
...
int main ()
{
    ...
    calculeScore(b, a);
    cout << "Nombre de 'O' : " << compteurPions(b, 'O') << endl;
